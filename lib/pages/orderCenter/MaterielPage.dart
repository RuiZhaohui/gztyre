...
class _MaterielPageState extends State<MaterielPage> {
  ...
  Future _addMateriel(List<SubmitMateriel> list) {
    return Future.forEach(list, (item) async {
      await HttpRequest.addMateriel(
          item.AUFNR, item.MATNR, item.MAKTX, item.MENGE, widget.device.deviceCode, (res) {}, (err) {
        throw Error();
      });
    });
  }
...
//                    Navigator.of(context).pop(this._requireMaterielList);
                    this
                        ._addMateriel(this._requireMaterielList)
                        .then((val) {
                        showCupertinoDialog(
                            context: context,
                          ...
                          children: <Widget>[
                            ...
                            GestureDetector(
                              child: Text(
                                '手动添加物料',
                                style: TextStyle(
                                    color: Color.fromRGBO(44, 93, 187, 1),
                                    fontSize: 14),
                                overflow: TextOverflow.fade,
                              ),
                              onTap: () {
                                    ...
                                    builder: (BuildContext context) {
                                      return CupertinoAlertDialog(
                                        content: CupertinoTextField(
                                          controller: _editSearchMaterielController,
                                          placeholder: "请输入物料编码",
                                        ),
                                        actions: <Widget>[
                                          CupertinoDialogAction(
                                          ...
                                          CupertinoDialogAction(
                                            onPressed: () {
                                              SubmitMateriel submitMateriel =
                                              new SubmitMateriel();
                                              submitMateriel.MAKTX = "";
                                              submitMateriel.MATNR = _editSearchMaterielController.text;
                                              submitMateriel.AUFNR =
                                                  widget.AUFNR;
                                              submitMateriel.MENGE = 0;
                                              this._isMaterielExist(submitMateriel.MATNR).then((success) {
                                                if (success) {
                                                  this
                              ...
                              onTap: () {
                                showCupertinoDialog(
                                    ...
                                    builder: (BuildContext context) {
                                      return CupertinoAlertDialog(
                                        content: CupertinoTextField(
                                          controller: _editMaterielController,
                                          placeholder: "请输入物料名称",
                                        ),
                                        actions: <Widget>[
                                          CupertinoDialogAction(
